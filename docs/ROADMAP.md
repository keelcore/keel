...
### Automatic SBOM Embedding

**What it is:** Embed the Software Bill of Materials (SBOM) directly into the Keel binary at build time, readable via `keel --sbom`.

**Why it matters:** Currently, SBOMs are generated by CI and attached to GitHub Releases. Embedding the SBOM in the binary means any running Keel instance carries its own provenance, even in air-gapped environments without internet access to the release page. This supports supply chain transparency requirements (Executive Order 14028, SLSA, NTIA minimum elements).

**Implementation:** `go generate` embeds a CycloneDX SBOM as a compressed byte slice. `keel --sbom` writes the SBOM to stdout.
